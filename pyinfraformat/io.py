...
def write_body(hole, f, comments=True, illegal=False):
    ...
    for line_dict in hole.survey.data:
        line_string = " ".join([str(value) for key, value in line_dict.items() if key != 'linenumber'])
    ...
    if comments:
        for comment_head, comment_dict in hole.inline_comment.data:
            line_string = "  " + str(comment_head) + " " + " ".join([str(value) for key, value in comment_dict.items() if key != 'linenumber'])
